#!/usr/bin/env bash
# Launch with: tmux new-session -d -s m5-e6-smoke 'bash scripts/m5/run_e6.sh smoke'
set -euo pipefail
...
case "$MODE" in
  smoke)
    RUN_NAME="${RUN_NAME:-e6_nomask_smoke_${STAMP}}"
    MODE_ARG="--smoke"
    ;;
  full)
    RUN_NAME="${RUN_NAME:-e6_nomask_${STAMP}}"
    MODE_ARG=""
    ;;
  *)
...
conda run --no-capture-output -n toolcredit \
  ...
  --config rl/configs/e6_nomask.yaml \
