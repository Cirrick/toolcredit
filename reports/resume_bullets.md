# 简历 bullets（2026-09-04 草稿，数字均来自 canonical 产物）

> 证据指针：M7 `eval/runs/m7_unified_eval_20260824_201032/metrics/`、`transitions/bootstrap.json`；
...
**多轮工具调用 RL 中的信用分配研究（个人项目，veRL + GRPO）** 2026.07–2026.09
...
- 以零侵入 wrapper（不 fork 框架）实现轮级信用分配 `A_turn = A_traj + β(s_t − s̄_t)`，对 10.2 万条训练轨迹、
  ...
  等价于隐式稠密奖励，并在约半数组内零方差 group 中成为唯一梯度来源。
...
- Pre-registered, hash-frozen evaluation: 4 checkpoints × 760 questions × (greedy + 4 samples) = 15,200
  ...
  ships resolved config, checkpoints, predictions, metrics, recovery ledgers, and 137 tests.
