...
## 12. M9 — E3-NoTool：工具的因果对照（完成：2026-09-07）

### 12.1 动机：项目最大的因果空洞

到 M8 为止，所有 RL 结果都在 TIR 协议下取得。SFT→E3 的 +12.76pt 是项目的招牌数字，但它无法回答
面试里最直接的一个问题：**这 12.76pt 里有多少是"学会用工具"？** `analysis/tool_use_conditional_accuracy.json`
只能给相关性（E3 有调用轨迹 .774 vs 零调用 .619），而"策略自己决定调不调"意味着两组题根本不可比。
M9 用最朴素的办法补上：同起点、同数据、同配方、同 200 步，**只把工具拿掉**，再训一条臂。

### 12.2 方法：零框架改动的 treatment

这是项目第一个不需要任何 veRL patch/子类/包裹的实验臂——treatment 就是"不给工具"：
`multi_turn.enable=false` + 原生 `single_turn_agent` + `agent_loop_config_path=null`。
resolved-config diff gate 断言 E3→E3-NoTool **恰好**差 5 条路径（集合相等，不是包含）。
Fixture J 证明无工具 prompt 逐 token 等于 `apply_chat_template(messages, tools=None)`，
且同一 tokenizer 下 TIR prompt 仍逐字节复现 E3 的 canonical `input`——两条臂的唯一差别就是 system 块里那段 schema。

评测侧协议 bump 到 v5：新增 `generation_mode`（`tir` 逐字段等于 v4；`notool` 为无 schema 单轮生成）
与四个 notool 角色（raw/SFT/E3 复用冻结权重，E3-NoTool 为新物化）。
语义验证证明 v4 的 87 文件闭包逐字节不变、12 个冻结语义字段完全相同。

### 12.3 结果：情形 B，以及一个精确闭合的分解

primary：Δ = E3-NoTool − E3 = **−0.92pt**，95% CI [−3.55, +1.71]，跨 0 → 预注册**情形 B（无差异）**。

真正有信息量的是四个 pass@1 构成的路径分解（同 760 题，逐项相加严格闭合到 +12.76pt）：

| 环节 | Δ | 占比 | CI |
|---|---|---|---|
| SFT-TIR .605 → SFT-NoTool .666（起点被工具拖累） | +6.05pt | 47% | 无预注册 CI |
| SFT-NoTool .666 → E3-NoTool-eval .713（RL 提升自身推理） | +4.74pt | 37% | [+2.11, +7.50] |
| E3-NoTool-eval .713 → E3-TIR .733（工具的推理时边际价值） | +1.97pt | 15% | [−4.74, +0.79] |

**SFT→E3 的 +12.76pt 里，近一半是"学会不再被工具拖累"，三分之一强是真实的推理提升，
只有 15% 是工具本身的边际价值，且不显著。** 工具学习确实发生了——工具的代价从 raw 的 −13.95pt
收窄到 SFT 的 −6.05pt 再到 E3 的 −1.97pt——但它买到的是**回到无工具基线**，不是超过它。

level 分解与 M1 的先验吻合：E3 相对 E3-NoTool 的优势只存在于 MATH L4（+7.03pt）与 L5（+2.99pt），
L1–2 与 GSM8K 打平或反向。这既是"工具在这个训练池里能用武之地很窄"的证据，
也说明 FULL760 的 null 结果不是"到处都没差别"，而是"少数难题上的正贡献被简单题上的负担抵消"。

### 12.4 一个反直觉的观察：无工具臂的梯度信号更差

组内 score 零方差比例：E3-NoTool 44.6%（前 25 步）→ 56.3%（后 25 步），而 E3 在 step 1 只有 26.6%。
去掉工具后策略更"尖"——全对组与全错组两头都变多。也就是说，**无工具臂在一半 batch 上没有任何梯度信号**，
却仍然追平了有工具臂。这是 M10（零方差过滤）最直接的动机：如果一半算力打在零梯度的 batch 上还能打平，
把这部分算力换成有信息的样本，理论上还有空间。

### 12.5 面试叙事

- **问"你的 RL 提升了 12.76pt，是模型变强了还是学会用工具了？"** → 我做了因果对照，答案是**主要是模型变强了**：
  同配方去掉工具，终点打平（−0.92pt，CI 跨 0）；E3 权重拿掉工具仍比 SFT 起点高 4.74pt，能力在权重里。
  12.76pt 可以精确拆成 6.05（不再被工具拖累）+ 4.74（推理提升）+ 1.97（工具边际价值，不显著）。
- **问"那工具是不是没用？"** → 在这个配置下（1.7B、≤4 轮、MATH 池）净贡献不显著，但**不是到处没用**：
  MATH L4–5 上工具值 +3 到 +7pt，被简单题上的负担抵消。要让工具净赚，得换训练池（长程符号/多步数值）
  或换规模，而不是调 RL 超参。
- **方法论的点**：null 结果之所以可信，靠的是预注册（判读矩阵先写、`D_start` 扣减规则先定）、
  唯一变量（config diff 恰好 5 条）、以及在看到终点**之前**就记录下"smoke 显示起点其实偏向 NoTool、
  因此不得引用保守方向"这条反向证据（`plans/M9.md` §11，2026-09-06）。
- **工程的点**：六条 200-step run 全部在 5–9 小时后被"pod 中断"，我查出根因是容器 128 GiB 上限 +
  `memory.oom.group=1` 下的组级 OOM，增长来自 FSDP CPU offload 的 pinned 内存池（只增不还）
  与 actor worker 的匿名内存（约 0.4 GB/step）；GH200 显存足够，offload 本就不该开（`reports/qa_log.md` Q17）。

## 13. M10 — E7：DAPO 式零方差 group 动态过滤（完成：2026-09-10）

### 13.1 动机：把已量化的病灶做成消融

到 M9 为止，两条 GRPO 臂都被同一个数字困扰：按进入优势的标量 `score` 计，E3 有 44% 的 group 组内零方差（末 25 步 53%），
无工具臂 45%→56%。整组优势为 0，rollout 白采，序列还留在 `token-mean` 分母里稀释梯度。DAPO 的动态采样正是治这个的：
丢零方差组，用同一冻结 snapshot 补采样到满 batch。M10 只做这一件（Clip-Higher 与 token-level loss 已在 E3 基线里，
overlong shaping 属 E4 领地，去 KL 信息量太低——`reports/qa_log.md` Q22），并预先写死了两条 x 轴与四格判读矩阵。

### 13.2 方法：pin `fit()` 的 409 行复制与 8 处白名单

这是项目对框架侵入最深的一次改动，也是纪律最严的一次。`rl/custom/dynamic_filter_trainer.py` 的 `fit()` 由脚本从 pin
veRL 0.8.0 `ray_trainer.py:1362–1770` 生成，只允许 STEP0 §3 的 8 处差异（每处 `# E7-W<n>`），Fixture R 逐行 diff 证明其余
逐字节相同；helper 内每个 chunk 逐字复用上游生成/合并行，old-log-prob 之后的路径一行不动。驱动边界用 E5 同款的模块名绑定
（0 行复制，proof 文件 + `finally` 恢复）。config diff gate 断言 E3→E7 **恰好**差 7 条路径。评测协议 bump 到 v6，
新增两个 TIR role，语义校验证明 13 个冻结字段（含整个 `generation_mode` 块）逐字节不变；**评测侧零框架改动**。

两条基础设施决定值得记：关闭 FSDP CPU offload（三条 diff，`rl/runs/e3_offload_check_*` 先验证数值可比）、每 50 有效 step
计划性分段恢复。200 步 4 段全部无 OOM，是项目第一条没有"意外 pod 中断"的 200-step run。

### 13.3 结果：情形 C，外加一个必须并列报告的 +5pt

| | 累计 rollout | update | greedy FULL760 | vs E3（配对，10k bootstrap） |
|---|---|---|---|---|
| E3 step-200 | 102,400 | 200 | .733 | — |
| E7 equal-compute（step 95） | 102,400 | 95 | .729 | **−0.39pt [−2.89, +2.11]**（Δ_compute） |
| E7 step-200 | 259,072（2.53×） | 200 | **.784** | +5.13pt [+2.63, +7.63]（附报） |

Δ_step（fixed-100 曲线 0–200 归一化 AUC）+1.44pt [−1.06, +4.13]；换成累计轨迹轴后 −1.72pt。两轴主判据都"持平"→**情形 C**：
E3 的零方差组对学习曲线影响有限，**每单位 rollout 算力的效率没有提高**。

但 step-200 终点 +5.13pt 显著为正（sampled +3.68pt），增益集中在 MATH L5（+7.5pt）与 AIME（各 +13/+17pt，n=30）。
这两组数字的关系是本章最重要的一句话：**动态采样在这里不是"更聪明地用同样的算力"，而是"把 E3 本来不会采的 1.53 倍
rollout 真的采出来训进去"**——一个用算力换终点的旋钮。它是否比"E3 多训 100 步"更好，本计划没有那条臂，不能回答。

训练侧的独立发现：**过滤的成本随策略变准单调上升**——零方差比例 34.5%→61.4%、全对组 24%→50%、每步批数 2.0→3.06；
外推到约 75% 就会 underfilled。DAPO 原文"总时间没有显著增加"在本设置下不成立。

行为上 E7 两个 checkpoint 都比 E3 少调工具（.63–.67 vs .89），step-200 的调用质量更高（成功率 93% vs 88%，重复代码 0.8% vs 2.9%，
截断 6.8% vs 9.6%），没有 over-calling——过滤改的是训练分布，不是奖励，这些是相关性。

### 13.4 匿名内存泄漏：定位到进程

memwatch v2 按进程 `Pss_Anon` 分 4 段拟合：cgroup anon 0.505/0.538/0.543/0.558 GiB/有效 step，**93.4% 来自单个
`ray::WorkerDict`**（actor 与 ref 共置的 FSDP worker），AgentLoop/sglang/驱动各 ≤ 0.003 GiB/step。这否定了 2026-09-09 的猜测
（轨迹缓存/沙箱残留），也说明关 offload 只降基线（49→25 GiB）不改斜率。按计划只诊断不修（`reports/qa_log.md` Q26）。

### 13.5 面试叙事

- **问"你复现 DAPO 动态采样了吗，有用吗？"** → 做了单因子消融，答案分两半：**等算力下没有免费午餐**（−0.4pt，CI 跨 0；
  轨迹轴 AUC 还低 1.7pt），但它是一个能把额外 rollout 换成终点的旋钮（2.53× rollout → +5.1pt，集中在难题）。
  我不会说"E7 更强"，因为缺 E3 同预算臂；我会说"论文按训练步数画曲线、一句话带过总时间，我把两条 x 轴都画了，结论就变了"。
- **问"为什么越训越贵？"** → 被学会的题从 mixed 变全对，零方差比例从 34% 涨到 61%，凑满一个 batch 要 3 批；
  这是 curriculum 的另一面——informative 集合向能力边界收缩，rollout 的边际信息量在下降。
- **方法论的点**：预注册的四格矩阵 + 两条 x 轴让 +5.13pt 这种"漂亮数字"没有机会被写成"更快"；等算力 checkpoint 是训练时按
  累计轨迹数自动落盘的，不是事后挑的。
- **工程的点**：复制 409 行框架代码但只允许 8 处差异、由测试逐行 diff 守着；六条 run 的"pod 中断"最终定位到一个进程的
  0.56 GiB/step 匿名增长，靠的是按进程 `Pss_Anon`、按重启分段、剔除启动期——任何一步偷懒都会给出错一个量级的斜率。
