...
| M10 E7 DAPO 式零方差过滤 | ✅ 完成（2026-09-10） | 只加动态采样：丢 `std(score)=0` 的组、同 snapshot 整批补采样（最多 4 批），E3→E7 config diff 精确 7 路径（含三条 offload 关闭）。200 有效 step 消耗 **259,072 条轨迹 = E3 的 2.53×**，零方差比例 34.5%→61.4%、每步批数 2.0→3.06（**过滤成本随策略变准单调上升**）。协议 v6 两个 role 7,600 条。greedy FULL760：E3 .733、**E7 等算力（step 95）.729**、E7 step-200 .784。预注册 **情形 C**：Δ_step（fixed-100 AUC）+1.44pt CI `[−1.06,+4.13]`、Δ_compute −0.39pt CI `[−2.89,+2.11]` 均持平，**每单位 rollout 算力无免费午餐**；附报 2.53× rollout 的终点 +5.13pt `[+2.63,+7.63]`（缺 E3 同预算臂，不归因于过滤）。匿名内存泄漏定位到 `ray::WorkerDict`（93.4%）。tag `m10`（详见 [main results M10 节](reports/02_main_results.md)、[plans/M10.md](plans/M10.md)） |
