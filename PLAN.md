...
判定不能只停留在 aggregate pass@1：还必须回答**各算法具体消除了哪些 failure case、引入了
哪些新 failure case，以及 credit assignment 是否特别改善了“同一轨迹中部分调用正确、部分
调用错误”这类标准轨迹级 GRPO 无法定位责任轮次的问题**。因此最终分析使用同一冻结题目集，
对 raw base、SFT、标准 GRPO、GRPO + turn-level credit 四个阶段做逐题配对的失败类型迁移分析。
...
**必须增加“信用是否真的分到了问题轮次”的机制验证**。标准轨迹级 GRPO 给同一轨迹的全部
生成轮次广播同一个 `A_traj`，无法精确指出是哪一次调用有问题；仅观察最终 pass@1 无法证明
E5 修复的是信用分配。为此：

1. 在 E3 与 E5 中保存可审计的 per-turn advantage（含 `A_traj`、`s_t`、组内基线和最终
   `A_turn(t)`），固定抽取至少 30 条同时含“有效调用 + 错误/未采纳调用”的真实多轮轨迹；
2. 对同一轨迹逐轮核对：E3 是否按设计给好/坏调用相同优势，E5 是否降低问题调用的相对信用、
   同时保留有效调用的信用；增加受控夹具测试钉住该不变量，shape 不符立即失败；
3. 把机制证据与 held-out 行为结果对应：重点报告调用 JSON 解析失败、无状态变量复用、工具执行
   报错后恢复、工具结果未采纳、调用预算耗尽/截断等类型在 E3→E5 后的修复率和新增率；
4. E5 必须从与 E3 相同的 SFT checkpoint **平行训练**，不得从 E3 终点继续训练，否则额外训练量
   会与 credit assignment 混淆。

完整的 raw base→SFT→E3/E5 四阶段 failure-case 对照按 §11 的冻结协议执行；M6 先完成 E3/E5
机制审计和同题配对，M7 再统一扩展到四个 checkpoint。

**两种结果都写成结论**：
- 若 E5 更好 → 细粒度信用在多轮工具场景有效，但必须用逐题 failure transition 说明增益来自
  哪类轨迹（长轨迹？含报错恢复？无状态错误？预算耗尽？），不得只用总分归因。
- 若无差异/更差 → 诚实的否定性结论：候选解释包括轨迹太短（≤4 轮）组内方差已足够、启发式 s_t 信号噪声大、β 未调优；给出"什么条件下值得再试"（更长 horizon、真实 agent 任务）。**否定性结论 + 机制分析在面试中的价值不低于正向结果。**

**验收标准**：E5 至少一个 β 值完整 run；per-turn advantage 夹具测试通过；至少 30 条混合质量
多轮轨迹的 E3/E5 机制审计落盘；E3→E5 同题 failure transition 表含分母、修复数、新增数和
bootstrap 置信区间；对比图表 + 判读写入 `02_main_results.md`。若某类样本过少，不得据此宣称
“已解决”，只报告观察值并标记证据不足。
...
- **四阶段 checkpoint 对照（必做）**：raw Qwen3-1.7B（未训练）、SFT-6k、E3 标准 GRPO、
  E5 GRPO + turn-level credit。E3 与 E5 都从同一 SFT 起点训练，不把 E5 误写成从 E3 继续训练。
- 在看到 E5 输出前冻结 failure-case 诊断 panel、题目 ID、生成参数、verifier 版本和 taxonomy；
  四个 checkpoint 使用完全相同的工具协议、max turns、prompt、greedy/n=4 采样设置与随机种子。
  panel 同时覆盖自然 held-out 分布和预注册的压力类型，且不得从训练 split 挪题。
- 保存 panel 的**全部**轨迹而不只保存失败样本，使“原来失败→后来正确”的修复和“原来正确→
  后来失败”的回归都可计算；任何 verifier/分类器版本变化必须全量重算四阶段，不得混用口径。

### 11.2 分析产出

1. **`02_main_results.md`**：主表（各方法 × 各指标）+ 训练曲线合图 + E5 判读 + E6 演示节。
2. **`03_badcase_taxonomy.md`**：对 raw base、SFT、E3、E5 的同一冻结 panel 做逐题配对分析，
   不再只比较 E3/E5 各自抽出的非配对失败样本。taxonomy 在看 E5 结果前冻结，至少覆盖：
   调用 JSON/解析失败、未调用工具（另分“答案正确”和“答案错误且工具可能有用”）、无状态变量
   复用、Python/沙箱执行错误、报错后未恢复、工具结果未采纳或误读、工具正确但语义推理/约束
   遗漏、精确答案被改成低精度近似、boxed/verifier 格式错误、工具滥用、预算耗尽/截断。
   每条轨迹记录一个 primary cause 和可多选 secondary tags；`badcase_taxonomy.py` 自动粗分，
   每个 checkpoint 分层人工抽检并统一裁决边界。
3. **failure transition 产物**：输出 raw→SFT、SFT→E3、E3→E5 的同题迁移矩阵和典型配对
   轨迹。每类至少报告：(a) 无条件发生率；(b) 条件修复率
   `先前为该类失败且后来正确 / 先前该类失败数`；(c) 新增失败率；(d) 从一种失败迁移到另一种
   失败的比例；(e) exact count 与按题 bootstrap 置信区间。E3→E5 是 credit assignment 的
   核心因果对照；raw→SFT 和 SFT→E3 分别回答 SFT、标准 GRPO 解决了什么。总分提高但关键
   failure case 不降，或旧失败变成新失败，都必须如实写入结论。
4. **`04_reward_hacking.md`**：整个项目期间收集的 hacking 案例（目标 ≥3 个，如：代码暴力枚举候选答案、刷执行成功分、boxed 空答案骗格式分），每例附原始轨迹、成因、verifier/奖励修复方式、修复后验证。
5. **`interview_outline.md`**：60 秒版与 5 分钟版叙事 + 每句话对应的数据索引。
...
| 各训练阶段具体解决了什么，而不只是总分涨了多少？ | 同一冻结 panel 上做 raw→SFT、SFT→E3、E3→E5 的逐题 failure transition；同时报告修复、新增和类型迁移，E3/E5 再用 per-turn advantage 审计连接机制与行为 | `03_badcase_taxonomy.md` + per-turn advantage 审计 | ✅ 核心因果解释 |
